...
import { Button, Segmented, Switch } from '../../runtime/ui/source-ui';
...
export function CanvasToolbar({
  ...
  onAddText: () => void;
  onAddConfig: () => void;
  threeD?: { readonly label: string; readonly onAdd: () => void };
  music?: { readonly label: string; readonly onAdd: () => void };
  onAddGroup: () => void;
  ...
  const extensionDefs = listNodeDefinitions().filter(
    (def) => def.showInCreateMenu !== false && getNodePluginId(def.type) !== 'builtin',
  );
  const dockStyle = {
  ...
  const tip = hovered === 'tool-music' ? music?.label : hovered === 'tool-3d' ? threeD?.label : hovered ? toolLabel(hovered, t) : '';
        ...
        >
          ...
          <div className='mt-3 flex items-center justify-between gap-3 rounded-lg px-1.5 py-1'>
            ...
            <Switch
              ...
              aria-label={t('canvas.toolbar.imageInfo')}
              className='data-[state=checked]:!bg-green-color'
            />
          </div>
        </div>
      ) : null}
    ...
    <Button
      ...
      style={
        active
          ...
          : hovered === id && !disabled
            ? hoverStyle
            : { color: danger ? 'var(--ui-destructive-color)' : theme.toolbar.item, opacity: disabled ? 0.35 : 1 }
      }
      icon={children}
      onMouseEnter={(event) => {
        onHover(id);
...
function CanvasThemeButton({
  ...
  const activeStyle =
    colorTheme === 'light'
      ? { background: 'var(--ui-canvas-accent)', color: 'var(--ui-canvas-accent-foreground)' }
