...
import { CONFIG_NODE_MIN_HEIGHT } from '../../constant/canvas';
...
const selectionColor = 'var(--ui-canvas-accent, var(--ui-text-color))';
...
type CanvasNodeProps = {
  ...
  isSelected: boolean;
  isRelated: boolean;
  isCurrentMusicResult?: boolean;
  musicResultHighlight?: InfiniteCanvasMusicResultHighlight;
  ...
  onAudioDurationChange?: (nodeId: string, url: string, durationMs: number) => void;
...
export const CanvasNode = React.memo(function CanvasNode({
  ...
  const pluginContext = useMemo<CanvasNodeContext | null>(
    () => (pluginHost ? buildNodeContext(pluginHost, data, theme, scale, isSelected) : null),
  ...
  const imageBorderColor = isActive ? selectionColor : isRelated ? theme.node.muted : 'transparent';
  ...
  const handleResizeMove = useCallback(
    (event: MouseEvent) => {
      ...
      const minWidth = data.type === CanvasNodeType.Audio && data.metadata?.generationMode === 'music' ? 400 : 220;
      const minHeight = data.type === CanvasNodeType.Audio && data.metadata?.generationMode === 'music' ? 280 : data.type === CanvasNodeType.Config ? CONFIG_NODE_MIN_HEIGHT : 160;
    ...
    [data.id, data.type, data.metadata?.generationMode, onResize, scale],
  );
  ...
  return (
    ...
    >
      {isCurrentMusicResult && (
        <div
          aria-hidden='true'
          data-current-music-result='true'
          style={{ position: 'absolute', inset: -5, borderRadius: 29, border: `2px solid ${musicResultHighlight?.color ?? theme.node.musicResultStroke}`, ...musicResultHighlight?.nodeStyle, pointerEvents: 'none' }}
        />
      )}
      ...
      <div
        ...
        style={{
          ...
          boxShadow: isGroupDropTarget
            ? `0 0 0 2px ${selectionColor}66, inset 0 0 0 999px ${selectionColor}10`
            : isActive
              ? `0 0 0 1px ${selectionColor}55`
              : isRelated
      ...
      >
        ...
        {!isGroup && data.type !== '3d' && !hasImageContent && !hasVideoContent && !hasAudioContent ? (
          <div
...
function NodeContent(props: NodeContentRendererProps) {
  if (props.node.metadata?.musicResultSource && props.renderNodeContent) return props.renderNodeContent(props.node);
  ...
  if (props.node.type === '3d' && props.renderNodeContent) return props.renderNodeContent(props.node);
  ...
  return (
    <div className='pointer-events-none flex h-full w-full flex-col p-4'>
      ...
      <div
        ...
        style={{ borderColor: theme.node.stroke, background: `${theme.node.fill}55` }}
      />
    </div>
  );
}
...
function AudioNodeContent({ node, theme, onAudioDurationChange }: NodeContentRendererProps) {
  const { t } = useInfiniteCanvasTranslation();
  if (!node.metadata?.content)
    return (
      <div
  ...
  if (node.metadata.musicRecordId || node.metadata.generationMode === 'music') return <CanvasMusicPlayer node={node} onDurationChange={onAudioDurationChange} />;
  return (
    <div
    ...
    >
      ...
      {image.content ? (
        <div className='absolute inset-x-2 top-2 flex items-center gap-1'>
          ...
          >
            <Star className='size-3 shrink-0' style={{ color: selectionColor }} />
            <span className='truncate'>{t('canvas.node.setPrimary')}</span>
          </button>
        </div>
      ) : null}
      {image.status === 'error' ? (
        <BatchImageFailureActions placement='right' onRetry={onRetry} onDelete={onDelete} />
      ) : null}
    </div>
  );
}
...
function ImageSlotStatus({ image }: { image?: CanvasNodeImage }) {
  ...
  return (
    <div
      ...
      style={{ background: theme.node.fill, color: failed ? theme.node.text : theme.node.activeStroke }}
    >
      {failed ? (
