...
import {
  ...
  type InfiniteCanvasStoreRegistry,
} from './editor-store-registry';
import {
  createPromptSourceStore,
  normalizePromptSourceState,
  type PromptSourceStore,
} from './use-prompt-source-store';
...
function hydrateAndPersistStores(stores: EditorStores, storageKeyPrefix: string): () => void {
  ...
  if (config !== null)
    stores.config.setState({
      config: normalizeConfig(
        { ...stores.config.getState().config, ...(config.config ?? (config as Partial<ConfigStore['config']>)), modelAdapter: undefined },
        stores.config.getState().config.modelAdapter,
      ),
    });

  const theme = readJson(key('theme')) as Partial<ThemeStore> | null;
  ...
  const sidePanel = readJson(key('preferences')) as
    | { readonly sidePanelOpen?: boolean; readonly sidePanelWidth?: number }
    | null;
  if (sidePanel !== null) {
  ...
  return combineCleanups([
    ...
    stores.theme.subscribe((state) => writeJson(key('theme'), { theme: state.theme })),
