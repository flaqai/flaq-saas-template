...
export function useCanvasIntegrations(openSettings: () => void): InfiniteCanvasIntegrations {
  ...
  return useMemo(() => ({
    ...
    navigateToDocs: () => window.location.assign('https://flaq.ai/docs?page=ai-canvas'),
    changeLocale: (locale) => router.replace(pathname, { locale }),
    confirm: ({ title, description }) => window.confirm(`${title}\n\n${description}`),
    toast: ({ kind, message }) => kind === 'error' ? toast.error(message) : kind === 'success' ? toast.success(message) : toast.info(message),
    onClientKeyRequired: openSettings,
    onProjectMissing: () => router.replace('/ai-canvas/projects'),
    onError: (error, operation) => {
      if (operation === 'load-project' && error instanceof CanvasStorageError && error.kind === 'missing') router.replace('/ai-canvas/projects');
      else toast.error(error instanceof Error ? error.message : String(error));
    },
    async resolveGenerationAccess() {
      try { await getClientOpenApiConfigAsync(); return { allowed: true }; }
      catch { return { allowed: false, reason: 'client-key-required' }; }
    },
    async upload({ files, signal }) {
      ...
      try {
        ...
        if (signal?.aborted) return { status: 'rejected', reason: 'cancelled' };
        if (urls.length !== files.length || urls.some((url) => !/^https?:\/\//i.test(url))) throw new Error(t('generation.failed'));
        return { status: 'success', files: files.map((file, index) => ({ name: file.name, mimeType: file.type, size: file.size, url: urls[index] })) };
      } catch (cause) {
        return { status: 'error', error: { code: 'upload-failed', message: cause instanceof Error ? cause.message : String(cause), cause } };
      }
    },
